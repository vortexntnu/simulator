...
void Dynamics::getConfig()
{
  ...
  R_euler << c(psi)*c(theta) << -s(psi)*c(phi)+c(psi)*s(theta)*s(phi) << s(psi)*s(phi)+c(psi)*s(phi)*s(theta) << arma::endr  // NOLINT(whitespace/line_length)
          << s(psi)*c(theta) << c(psi)*c(phi)+s(phi)*s(theta)*s(psi) << -c(psi)*s(phi)+s(theta)*s(psi)*c(phi) << arma::endr  // NOLINT(whitespace/line_length)
...
void Dynamics::updateCoriolisMatrices()
{
  ...
  C_a = arma::join_cols(arma::join_rows(zeros_3x3, -Dynamics::skewSymmetric(var1)),
    arma::join_rows(-Dynamics::skewSymmetric(var1), -Dynamics::skewSymmetric(var2)));
  ...
  C_rb = arma::join_cols(arma::join_rows(zeros_3x3, -Dynamics::skewSymmetric(var1)),
    arma::join_rows(-Dynamics::skewSymmetric(var1), -Dynamics::skewSymmetric(var2)));
